...
const handler = async (req, res) => {
    ...
    try {
        ...
        const sellerOrders = await OrdersModel.find({ 'products.product.sellerId': req.user.id, paymentStatus: true, deliveryStatus: false });
        ...
        let ciphertext = CryptoJS.AES.encrypt(JSON.stringify({ sellerProducts: newSellerProducts, sellerOrders: sellerOrders, productLength: monthlyProducts, orderLength: monthlyOrders, deliveredOrders: newDeliveredOrders, monthlyDelivered: monthlyDelivered }), process.env.JWT).toString();
