        ...
        <>
            ...
            <main className='w-full flex flex-col'>
                ...
                <div className='w-full md:w-7/12 mx-auto text-sm p-2 font-light items-center justify-center bg-white gap-4 pb-10 mt-20 flex flex-col'>
                    ...
                    <input value={form.title} name='title' onChange={handleForm} placeholder='Product Name' className='rounded placeholder:text-slate-400 w-full placeholder:font-medium font-medium text-sm border-2 border-slate-400 p-2 outline-none' />
                    <textarea onChange={handleForm} value={form.desc} name='desc' rows={8} placeholder='Product Description' className='rounded w-full placeholder:font-medium placeholder:text-slate-400 font-medium text-sm border-2 border-slate-400 p-2 outline-none' />
                    <div className='w-full grid grid-cols-1 md:grid-cols-3 gap-4'>
                        <div className='w-full flex items-center gap-2 rounded border-2 border-slate-400 p-2'>
                            <span className='text-sm text-slate-400 font-medium'>&#x20b9;</span>
                            <input onChange={handleForm} value={form.price} name='price' placeholder='Price' type='number' className=' w-full placeholder:font-medium font-medium text-sm  outline-none' />
                        </div>
                        <input onChange={handleForm} value={form.brand} name='brand' placeholder='Brand' type='text' className='flex items-center gap-2 rounded border-2 border-slate-400 p-2 w-full placeholder:font-medium font-medium text-sm  outline-none' />
                        <div onClick={() => setShowCategory(!showCategory)} className='w-full flex relative items-center cursor-pointer justify-between rounded text-slate-400 font-medium text-sm border-2 border-slate-400 p-2'>
                            ...
                            {showCategory && <div className='absolute top-10 md:p-2 p-1 rounded border-2 border-slate-400 bg-white z-20 w-full left-0 right-0 flex flex-col'>
                                {categoryList.map((e, i) => {
                                    return (
                                        <span onClick={() => setForm({ ...form, category: e })} className='text-sm md:p-2 p-1 font-medium rounded hover:bg-slate-300 text-black' key={`category-${i}`}>{e}</span>
                                    )
                                })}
                            </div>}
                        </div>
                    </div>
                    <span className='w-full font-medium'>Upload Images</span>
                    ...
                    <div className='w-full flex justify-end'>
                        <button onClick={handleSubmit} className='bg-red-500 w-full md:w-1/4 p-3 text-white text-sm font-medium rounded float-right hover:bg-red-600'>ADD NOW</button>
                    </div>
                </div>
                {loading && <Loading />}
            </main>
