...
function AccountDetail({title, value, className, valueField, setEdit}) {
    ...
    return (
        <div className={`w-full flex gap-4 px-3 py-4 justify-start items-start ${className}`}>
            ...
            {title === 'Name' && <button onClick={handleEdit} className='px-6 font-medium md:px-10 text-sm py-1 md:py-2 rounded-md border-2 hover:shadow-xl shadow-lg'>Edit</button>}
        </div>
    )
}
